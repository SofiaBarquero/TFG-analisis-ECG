...
%{
%Las variables que finalizan en “I” hacen referencia a la señal integrada y “F” hacen referencia a la señal filtrada.
...
for i=1:picos
    ...
    if loc_i(i)-round(0.15*fs)>=1 && loc_i(i)<=length(signal_f)
       ...
       x=x+loc_i(i)-round(0.150*fs);
        ...
        elseif locs(i)>=length(signal_f) %ultima
    ...
    %comparo umbrales
    if PEAKI>=round(THRESHOLDI1,3) %PEAKI is part of the possible QRS peaks
        ...
        if onda_t==0 %no se detecto onda T
            ...
            if PEAKF>round(THRESHOLDF1,3) %peak IS part of the QRS peaks
                beat_f=beat_f+1;
        ...
        if PEAKI>=round(THRESHOLDI2,3) %mayor que el umbral de ruido de la integrada
            if beat_f>0
